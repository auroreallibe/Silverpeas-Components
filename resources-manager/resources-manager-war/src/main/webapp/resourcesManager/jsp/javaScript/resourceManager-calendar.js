...
/**
 * Render the label of a period.
 * @param labels
 * @param filters
 */
function renderPeriodLabel(labels, filters) {
  var $periodLabel = $('<div>');
  $periodLabel.append(labels.monthNames[filters.beginPeriodDay.getMonth()]);
  if (filters.beginPeriodDay.getMonth() != filters.endPeriodDay.getMonth()) {
    if (filters.beginPeriodDay.getYear() != filters.endPeriodDay.getYear()) {
      $periodLabel.append(' ' + filters.beginPeriodDay.getFullYear());
    }
    $periodLabel.append(' / ' + labels.monthNames[filters.endPeriodDay.getMonth()]);
  }
  $periodLabel.append(' ' + filters.endPeriodDay.getFullYear());
  if (filters.weeklyView) {
    $periodLabel.append(' - ' + labels.week + ' ' +
        $.datepicker.iso8601Week(filters.beginPeriodDay));
  }
  return $periodLabel.html();
}
  ...
  if (reservationEvents && reservationEvents.length > 0) {
    ...
    $.each(reservationEvents, function(index, reservationEvent) {
      ...
      if (index == 0 ||
        ...
        $listing.append(renderWeekSeparation(labels, filters));
 ...
 * @param filters
 */
function renderWeekSeparation(labels, filters) {
  var filtersForSeparator = $.extend({}, filters);
  filtersForSeparator.monthlyView = false;
  filtersForSeparator.weeklyView = true;
  var $separation = $('<div>').addClass('week-separation');
  $separation.append(renderPeriodLabel(labels, filtersForSeparator));
  return $separation;
}
